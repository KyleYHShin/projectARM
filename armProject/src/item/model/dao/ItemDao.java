...
public class ItemDao {
	...
	public int addItemCount(Connection con, int itemNo) {
		...
		String query = "update item set item_count = " + "(select item_count from item where item_no = ?) + 1 "
				+ "where item_no = ?";
		...
		try {
			...
			System.out.println("아이템 조회수 처리 쿼리문 틀림");
			e.printStackTrace();
		} finally {
			close(pstmt);
		}
	...
	public ArrayList<Integer> selectSubItemNo(Connection con, int itemNo) {
		ArrayList<Integer> subItemNoList = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String query = "select item_sub_no from item_bridge" + " where item_no = ?";
		...
		try {
			...
			while (rset.next()) {
				if (flag == true) {
					subItemNoList = new ArrayList<Integer>();
					flag = false;
				}

				int subItemNo = rset.getInt("item_sub_no");
				subItemNoList.add(subItemNo);
			}

		} catch (Exception e) {
			System.out.println("서브아이템 번호 조회 쿼리 틀림");
			e.printStackTrace();
		} finally {
			close(rset);
			close(pstmt);
	...
	public ArrayList<SubItem> selectOneSub(Connection con, ArrayList<Integer> subItemNoList) {
		...
		try {
			...
			for(int i = 0; i < subItemNoList.size(); i++){
				pstmt = con.prepareStatement(query);
				pstmt.setInt(1, subItemNoList.get(i));
				rset = pstmt.executeQuery();
