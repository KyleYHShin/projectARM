...
<table>
		<tr>
			<th width="10%">문의번호</th><th width="200px">상품명</th><th>선택옵션명</th><th>문의등록일</th><th>답변여부</th>
		</tr>
		<% 
		...
		<tr>
			...
			<td><%= itemList.get(item_no) %></td>
			<td><%= subItemList.get(item_sub_no) %></td>
			<td><%= questionList.get(i).getqDate() %></td>	
			<td><%= answered %></td>
		</tr>
		<tr>
			<td id="<%= questionList.get(i).getqNo() %>"></td>
			<td colspan="4" style="background:#fffff0">
			문의 내용 보기
			</td>
		</tr>
		<tr class="content">
			<td id="<%= questionList.get(i).getqNo() %>"></td>
			<td colspan="4"><%= questionList.get(i).getqContent() %></td>
		</tr>
			<% if (answered.equals("Yes")){ %>
		<tr>
			<td id="<%= questionList.get(i).getqNo() %>"></td>
			<td colspan="4" style="background:#fffff0">답변 내용 보기</td>
		</tr>
		<tr class="content">
			<td id="<%= questionList.get(i).getqNo() %>"></td>
			<td colspan="3">
		...
		<% }else{ %>
			...
			<tr>
				<td id="<%= questionList.get(i).getqNo() %>"></td>
				<td colspan="4" style="background:#fffff0">답변 작성하기</td>
			</tr>
			<tr class="content">
				<td id="<%= questionList.get(i).getqNo() %>"></td>
				<td colspan="3">
